...
\paragraph{PECNET as a baseline on local data. Inferred, from the poster session.} The personal
notes record an intention to test the PECNET framework from poster B20 of
the poster session against a dataset of the note-taker's own. That is a concrete and cheap
experiment, listed here so that it is not lost.

\begin{aside}
One direction is deliberately omitted: anything derived from the topic of poster B34, Atomic Action
Slicing. Proposing directions for that project on the basis of lecture notes and a poster title alone
would be guesswork.
\end{aside}
