...
The lecture's organising move is to treat diffusion as one of two ways to break generation
into easy steps, and then to show that almost every design decision people argue about is
downstream of a single equation for the forward corruption process. Once
...
flow matching takes $\alpha = 1 - \sigma$. Presenting it this way is the most useful thing
in the lecture, because it collapses a literature that usually reads as three separate
methods into one line with a parameter.
...
classifier~\citep{nichol2022glide}. Figure~\ref{fig:diffusion-models-2} is worth studying,
because it makes clear that guidance is a geometric operation on one sampling step rather
...
\paragraph{Where the framework strains.} Three honest limitations close the lecture.
...
The deck writes the guidance scale as a Greek gamma; this book uses $\guide$ to avoid
collision with the discount factor $\disc$ in
Chapter~\ref{ch:04-intro-reinforcement-learning}, which is a deliberate departure from the
speaker's notation and the only one in this chapter.

\section*{Why it matters}

The parameterisation argument is the most portable idea here: whenever a literature presents
$n$ competing methods, it is worth asking whether they are $n$ choices of one coefficient.
...
\begin{aside}
Two threads worth pulling. The personal notes record the claim that diffusion models are
