...
a single output and thereby throws structure away. The construction is therefore a stack of
equivariant layers with an invariant readout only at the end. Whether something is a symmetry is a property of the task, not
...
\paragraph{Grids give CNNs, and the derivation is tight.} A grid is a graph with more
regularity, which narrows the permutations that matter down to the translations.
...
updates instead. Manifolds push this furthest. A 2D manifold is locally like $\R^2$, so the work
happens in the tangent space $T_u\Om$, movement along the surface goes through the exponential map
$\exp_u X$, and tangent vectors at different points are compared by parallel transport
$\Gamma_{u \to v}X$. Defining
...
This lecture retroactively organises several others. Cho's Set Transformer++ (Chapter~\ref{ch:05-statistics-estimation}) is a permutation
...
The dependency runs one way: nothing in the four Gs requires that background in order to be
\emph{used}, but the derivations, which are the actual content of the lecture, do.
\end{aside}
