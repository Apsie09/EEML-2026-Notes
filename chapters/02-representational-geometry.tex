...
compare kernels. Once comparison is on that footing, convergence becomes measurable:
strong models trained on different data and different modalities are converging in kernel
structure, and human perceptual judgements sit close to it. The lecture ends by turning this from an
observation into an engineering target, and states what that costs.
...
$\rep$ and $\repB$ denote representations, $z = \rep(\xin)$ an embedding, $Z$ a point cloud over a
dataset, $h$ a map fitted between two embeddings, $\kernel$ a kernel matrix and $\Hcent$ the centring
matrix.

\section*{Why it matters}

This is the lecture the personal notes cover in most detail, and it connects outward more than any
other. The claim that distance is the only thing to preserve is the same
...
converge becomes harder to state without circularity.
\end{aside}
