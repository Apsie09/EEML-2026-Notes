...
learning at all. The meta-problem is evaluation, and the lecture is candid that neither option
is satisfactory: train on a pre-specified sequence of tasks, which is artificial, or find an
application that is naturally continual, which is uncontrolled. That tension is worth keeping
in mind whenever a continual-learning result is quoted.
...
\paragraph{Forgetting is gradient conflict, and the fixes follow from that.} The intuition
given is geometric and is worth looking at directly
...
everything, which pairs with distillation. Lyle's practical verdict is blunt and appears again
in the summary: if the data can be kept, the first problem is easy. Only under a memory constraint
does it require finesse.

\paragraph{Loss of plasticity has a mechanism, and it is the optimiser.} This is the most
valuable part of the lecture because it is a causal chain rather than a phenomenon. Under
repeated task changes networks become less trainable over time, in the extreme case with dead
...
\paragraph{The fixes are boring, which is the point.} What Lyle calls the swiss cheese approach
...
constraint. That distinction is worth settling for any given project before reaching for a
continual-learning method, since the literature is largely written for the constrained case and
most applications are not in it.
\end{aside}
