...
The folder holding this deck is untitled on disk; the title above is the deck's own, and it is
accurate, because the lecture is a survey of open problems rather than a result. Its spine is
an argument about inductive bias: there is no prior-free machine learning system, every choice
...
assumption is itself a load-bearing bias, because gradient descent resolves credit assignment
by a tug-of-war between examples and that only works when the data is shuffled. Structurally,
depth and architecture are biases with quantifiable payoffs. Practically, state space
models and the linear recurrent unit are a worked example of buying capability from structure
rather than from scale, and the lecture is unusually honest that at scale most of the advantage
evaporates into a hybrid.
...
\paragraph{The i.i.d.\ assumption is doing hidden work.} This is the sharpest claim in the
lecture. Gradient-based credit assignment inside a function approximator proceeds by
tug-of-war dynamics between competing examples, which requires the data to be i.i.d. Two
consequences are offered as hypotheses rather than facts: there is no explicit knowledge
...
solution is reached, which Pascanu argues should be leveraged rather than treated as
...
His objection is methodological rather than empirical. There are many variants (S4~\citep{gu2022s4},
...
diagonalisation, so one learns a complex diagonal $\Lam$ directly and the recurrent computation
...
\paragraph{The honest scorecard, and the hybrid that actually wins.} Having built the case,
...
unavoidable. Both are right, and the interesting question is which biases are cheap to state and
expensive to learn. That is a serviceable filter for research ideas.
\end{aside}
