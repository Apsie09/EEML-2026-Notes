...
This is the tightest technical talk of the week: one obstruction, one construction, one proof
that the construction costs nothing. Graph signal processing supplies a Fourier transform by
...
$\Adj^{K} = \vect{0}$ and hence $\lambda_i = 0$ for every $i$. The lecture's own aside, bottom
right, is the one worth remembering: this is the same matrix structure as a causal attention mask,
so every decoder-only transformer has the same degenerate spectrum.}
...
attention mask, is the connection worth chasing. It means every decoder-only transformer is a DAG
...
\begin{aside}
The exhaustive diagonalisability check is the part worth imitating. The construction's soft spot
is not the spectrum but the requirement of distinct eigenvalues, and rather than assume genericity
the talk enumerates all $2^{21}$ connected DAGs on eight vertices and reports the failure rate. A
percentage from a complete enumeration on small instances is much stronger evidence than an
asymptotic argument, and it costs an afternoon.
\end{aside}
